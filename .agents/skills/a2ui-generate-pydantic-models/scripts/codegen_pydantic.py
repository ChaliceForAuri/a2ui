...
"""Dynamic, version-agnostic automated generator for Pydantic v2 schemas, basic catalogs, and adapters across any A2UI spec version."""
...
def normalize_version(raw_version: str) -> Tuple[str, str, str]:
    """
    Normalizes any version input string.
    Returns a tuple of:
    - dir_name: e.g. "v1_1", "v0_9", "v0_9_1"
    - spec_dot: e.g. "v1.1", "v0.9", "v0.9.1"
    - enum_name: e.g. "V1_1", "V0_9", "V0_9_1"
    """
    clean = raw_version.strip()
    if clean.startswith("v") or clean.startswith("V"):
        clean = clean[1:]

    parts = re.split(r"[._-]", clean)
    dir_name = "v" + "_".join(parts)
    spec_dot = "v" + ".".join(parts)
    enum_name = "V" + "_".join(parts)

    return dir_name, spec_dot, enum_name
...
def generate_constants_code(
    dir_name: str,
    spec_dot: str,
    s2c_data: Dict[str, Any],
    c2s_data: Dict[str, Any],
) -> str:
    ...
    if defs:
        ...
        for k in s2c_data.get("properties", {}).keys():
            if k != "version" and k not in outbound_keys:
                outbound_keys.append(k)
    ...
    inbound_props = c2s_data.get("properties", {})
...
class PydanticCodegen:
    ...
    def __init__(self, dir_name: str, spec_dot: str):
        self.dir_name = dir_name
        self.spec_dot = spec_dot
...
def generate_server_to_client(
    s2c_data: Dict[str, Any],
    dir_name: str = "v0_9",
    spec_dot: str = "v0.9",
) -> Tuple[str, List[str]]:
    """Generates server_to_client.py / agent_to_renderer.py content and message names."""
    codegen = PydanticCodegen(dir_name, spec_dot)
    codegen.allow_inline = False
    is_modern_terminology = dir_name not in ("v0_8", "v0_9")
    ...
    defs_s2c = s2c_data.get("$defs", {})
    msg_names = []
    if defs_s2c:
        for mname, mschema in defs_s2c.items():
    ...
    union_name = (
        "AgentToRendererMessage" if is_modern_terminology else "ServerToClientMessage"
    )
    if msg_names:
        union_lines = [f"{union_name} = Union["]
        for mn in msg_names:
            union_lines.append(f"    {mn},")
        union_lines.append("]")
        s2c_blocks.append("\n".join(union_lines))

    return "\n\n\n".join(b.strip() for b in s2c_blocks if b.strip()) + "\n", msg_names


def generate_client_to_server(
    c2s_data: Dict[str, Any],
    dir_name: str = "v0_9",
    spec_dot: str = "v0.9",
) -> str:
    """Generates client_to_server.py / renderer_to_agent.py content."""
    codegen = PydanticCodegen(dir_name, spec_dot)
    ...
    c2s_imports = f"from .common_types import {', '.join(needed_imports)}\n"
    ...
    # Errors
    err_key = "error" if "error" in props else None
    err_classes = []
    if err_key:
        err_spec = props[err_key]
        if "oneOf" in err_spec or "anyOf" in err_spec:
            ...
            for item in items:
                ...
                c2s_blocks.append(codegen.compile_object_def(class_name, item))
                err_classes.append(class_name)
        elif "properties" in err_spec:
            c2s_blocks.append(codegen.compile_object_def("A2uiClientError", err_spec))
            err_classes.append("A2uiClientError")

    if err_classes:
        c2s_blocks.append(f"A2uiClientError = Union[{', '.join(err_classes)}]\n")
        c2s_blocks.append("A2uiRendererError = A2uiClientError")

    c2s_blocks.append(
        "class A2uiClientActionMessage(StrictBaseModel):\n    version:"
        " SPEC_VERSION_TYPE = SPEC_VERSION\n    action: A2uiClientAction ="
        ' Field(..., description="Action dispatched.")'
    )
    c2s_blocks.append(
        "class A2uiClientErrorMessage(StrictBaseModel):\n    version:"
        " SPEC_VERSION_TYPE = SPEC_VERSION\n    error: A2uiClientError ="
        ' Field(..., description="Client error.")'
    )
    c2s_blocks.append(
        "A2uiClientMessage = Union[A2uiClientActionMessage, A2uiClientErrorMessage]"
    )

    return "\n\n\n".join(b.strip() for b in c2s_blocks if b.strip()) + "\n"
...
def generate_client_capabilities(
    ...
    """Generates client_capabilities.py content."""
    codegen = PydanticCodegen(dir_name, spec_dot)
    caps_blocks = [
        (
            f"{FILE_HEADER}\n"
...
def generate_schema_init(
    ...
    for mn in msg_names:
        ...
        ver_init.append(f"    {mn} as {mn},")
        if payload != mn:
            ver_init.append(f"    {payload} as {payload},")
    ver_init.extend([
        ")",
        "from .constants import *",
    ])
    if is_modern_terminology:
        ver_init.append("from .agent_to_renderer import *")
        ver_init.append("from .renderer_to_agent import *")
        ver_init.append("from .renderer_capabilities import *")
    else:
        ver_init.append("from .server_to_client import *")
        ver_init.append("from .client_to_server import *")
        ver_init.append("from .client_capabilities import *")
...
def generate_basic_catalog_functions(
    ...
    """Generates functions.py content and generated class names."""
    codegen = PydanticCodegen(dir_name, spec_dot)
    func_blocks = [
        (
            f"{FILE_HEADER}\nfrom typing import Any, Dict, List, Literal, Optional,"
    ...
    basic_funcs_lines = ["BASIC_FUNCTIONS = ["]
    for aname in api_func_names:
        basic_funcs_lines.append(f"    {aname}(),")
    basic_funcs_lines.append("]")
    func_blocks.append("\n".join(basic_funcs_lines))
    names.append("BASIC_FUNCTIONS")
...
def generate_version_schemas(
    ...
    s2c_path = os.path.join(json_dir, s2c_name)
    s2c_data = {}
    if os.path.exists(s2c_path):
        with open(s2c_path, "r", encoding="utf-8") as f:
    ...
    c2s_path = os.path.join(json_dir, c2s_name)
    c2s_data = {}
    if os.path.exists(c2s_path):
        with open(c2s_path, "r", encoding="utf-8") as f:
            c2s_data = json.load(f)

    # 1. Generate constants.py dynamically from schemas
    constants_path = os.path.join(out_dir, "constants.py")
    constants_code = generate_constants_code(dir_name, spec_dot, s2c_data, c2s_data)
...
CallId = str""",
    ...
    is_modern_terminology = "agent_to_renderer" in s2c_name or dir_name not in (
        "v0_8",
        "v0_9",
    )
    ...
    if s2c_data:
        ...
        if defs_s2c:
            for mname, mschema in defs_s2c.items():
                ...
                snake_env = to_snake_case(envelope_key)
                alias_opt = (
                    f', alias="{envelope_key}"' if snake_env != envelope_key else ""
                )
                s2c_blocks.append(
                    f"class {mname}(StrictBaseModel):\n"
                    "    version: SPEC_VERSION_TYPE = SPEC_VERSION\n"
        ...
        if msg_names:
            if is_modern_terminology:
                s2c_blocks.append(
                    f"AgentToRendererMessage = Union[{', '.join(msg_names)}]"
                )
                s2c_blocks.append(
                ...
                s2c_blocks.append(
                    f"ServerToClientMessage = Union[{', '.join(msg_names)}]"
                )
                s2c_blocks.append(
        ...
        out_file_name = (
            "agent_to_renderer.py" if is_modern_terminology else "server_to_client.py"
        )
    ...
    if c2s_data:
        ...
        if "callAgentFunction" in props:
            c2s_blocks.append(
                codegen.compile_object_def(
                    "CallAgentFunction", props["callAgentFunction"]
                )
            )

        if "error" in props:
            error_spec = props["error"]
            if "oneOf" in error_spec:
                error_variants = error_spec.get("oneOf", [])
                err_names = []
                for v in error_variants:
                    title = v.get("title", "Generic Error")
                    if "Validation" in title:
                        cname = "A2uiValidationError"
                    else:
                        cname = "A2uiGenericError"
                    c2s_blocks.append(codegen.compile_object_def(cname, v))
                    err_names.append(cname)
                c2s_blocks.append("A2uiValidationFailedError = A2uiValidationError")
                if err_names:
                    c2s_blocks.append(
                        f"A2uiClientError = Union[{', '.join(err_names)}]"
                    )
            else:
                c2s_blocks.append(
                    ...
                    "    message: Optional[str] = Field(None)\n\n"
                    "A2uiClientError = A2uiGenericError"
                )
                c2s_blocks.append(
                    "class A2uiValidationError(A2uiGenericError):\n    pass"
                )

        msg_union_members = []
        if action_key:
            snake_act = to_snake_case(action_key)
            alias_opt = f', alias="{action_key}"' if snake_act != action_key else ""
            action_type = (
                "A2uiRendererAction" if is_modern_terminology else "A2uiClientAction"
            )
            c2s_blocks.append(
                "class A2uiRendererActionMessage(StrictBaseModel):\n"
                "    version: SPEC_VERSION_TYPE = SPEC_VERSION\n"
                f"    {snake_act}: {action_type} = Field(...{alias_opt})"
            )
            c2s_blocks.append(
                "A2uiClientActionMessage = A2uiRendererActionMessage\n"
                "A2uiClientUserActionMessage = A2uiRendererActionMessage"
            )
            msg_union_members.append(
                "A2uiRendererActionMessage"
                if is_modern_terminology
                else "A2uiClientActionMessage"
            )
        ...
        union_def = (
            f"Union[{', '.join(msg_union_members)}]" if msg_union_members else "Any"
        )

        if is_modern_terminology:
            c2s_blocks.append(f"RendererToAgentMessage = {union_def}")
            c2s_blocks.append(
                "ClientToServerMessage = RendererToAgentMessage\n"
                "A2uiClientMessage = RendererToAgentMessage"
            )
        else:
            c2s_blocks.append(f"ClientToServerMessage = {union_def}")
            c2s_blocks.append(
                "RendererToAgentMessage = ClientToServerMessage\n"
                "A2uiClientMessage = ClientToServerMessage"
            )

        union_name = (
            "RendererToAgentMessage"
            if is_modern_terminology
            else "ClientToServerMessage"
        )
        c2s_blocks.append(
        ...
        c2s_blocks.append(f"A2uiClientMessageList = List[{union_name}]")
        c2s_blocks.append(
        ...
        out_c2s_name = (
            "renderer_to_agent.py" if is_modern_terminology else "client_to_server.py"
        )
        with open(os.path.join(out_dir, out_c2s_name), "w", encoding="utf-8") as f:
            f.write("\n\n\n".join(b.strip() for b in c2s_blocks if b.strip()) + "\n")
    ...
    if os.path.exists(caps_path):
        ...
        v_key = spec_dot
        v_props = caps_data.get("properties", {}).get(v_key, {}).get("properties", {})
        v_req = caps_data.get("properties", {}).get(v_key, {}).get("required", [])
...
def generate_basic_catalog(
    ...
    if common_types_json_path:
        with open(common_types_json_path, "r", encoding="utf-8") as f:
            common_defs = json.load(f).get("$defs", {})

    all_defs = dict(common_defs)
    all_defs.update(cat_data.get("$defs", {}))
        ...
        if "allOf" in args_schema:
            for sub_item in args_schema["allOf"]:
                if isinstance(sub_item, dict):
                    args_props.update(sub_item.get("properties", {}))
                    args_req.extend(sub_item.get("required", []))
        pascal_fname = to_pascal_case(fname)
        args_class = f"{pascal_fname}Args"

        if args_props:
            func_blocks.append(
                codegen.compile_object_def(
                    args_class, {"properties": args_props, "required": args_req}
                )
            )
        else:
            args_class = "None"

        return_type = fprops.get("returnType", {}).get("const", "any")
        api_class = f"{pascal_fname}Api"
        func_blocks.append(
            f"class {api_class}(FunctionApi):\n"
            f'    name = "{fname}"\n'
            f"    schema = {args_class}\n"
            f'    return_type = "{return_type}"'
        )
        api_func_names.append(api_class)

    with open(os.path.join(out_dir, "functions.py"), "w", encoding="utf-8") as f:
        f.write("\n\n\n".join(b.strip() for b in func_blocks if b.strip()) + "\n")
    ...
    styles_blocks = [
        (
            ...
            "from pydantic import BaseModel, Field, ConfigDict\n"
            f"from ...schema.{dir_name}.common_types import StrictBaseModel"
        ),
    ]
    theme_spec = cat_data.get("$defs", {}).get("theme", {})
    if theme_spec:
        styles_blocks.append(codegen.compile_object_def("Theme", theme_spec))
    else:
        styles_blocks.append(
            'class Theme(BaseModel):\n    model_config = ConfigDict(extra="allow")'
        )

    with open(os.path.join(out_dir, "styles.py"), "w", encoding="utf-8") as f:
        f.write("\n\n\n".join(b.strip() for b in styles_blocks if b.strip()) + "\n")
...
def generate_adapter_scaffold(
    ...
    """Generates an adapter scaffold in src/a2ui/core/processing/adapters/<version>.py."""
    o_root = out_root or CORE_SRC_ROOT
    out_dir = os.path.join(o_root, "processing/adapters")
    os.makedirs(out_dir, exist_ok=True)
    adapter_file = os.path.join(out_dir, f"{dir_name}.py")

    if os.path.exists(adapter_file):
        return

    adapter_cls_name = f"{enum_name}Adapter"
    is_v08 = dir_name == "v0_8"
    is_v09 = dir_name in ("v0_9", "v0_9_1")

    if is_v08:
        prop_extractor = 'payload.get("styles") or {}'
    else:
        prop_extractor = 'payload.get("theme") or {}'
...
class {adapter_cls_name}(VersionAdapter):
    \"\"\"Protocol version adapter for A2UI {spec_dot}.\"\"\"

    @property
    def version(self) -> A2uiProtocolVersion:
        return A2uiProtocolVersion.{enum_name}

    def extract_theme(self, payload: Dict[str, Any]) -> Dict[str, Any]:
...
def update_root_schema_init(
    known_versions: List[Tuple[str, str, str]],
    out_root: Optional[str] = None,
) -> None:
    """Dynamically regenerates src/a2ui/core/schema/__init__.py with all known versions."""
    o_root = out_root or CORE_SRC_ROOT
    imports_lines = [f"from . import {d} as {d}" for d, _, _ in known_versions]

    enum_members = []
    for _, s_dot, e_name in known_versions:
        enum_members.append(f'    {e_name} = "{s_dot}"')

    agent_union_items = []
    renderer_union_items = []
    for d, _, _ in known_versions:
    ...
    latest_dir = known_versions[-1][0] if known_versions else "v0_9"
...
"""
        ...
        preferred_reexport = (
            "v0_9" if any(d == "v0_9" for d, _, _ in known_versions) else latest_dir
        )
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python codegen_pydantic.py --version v0.8   # Generate models for v0.8\n"
            "  python codegen_pydantic.py --version v0.9   # Generate models for v0.9\n"
            "  python codegen_pydantic.py --version v1.0   # Generate models for v1.0\n"
            "  python codegen_pydantic.py --version v1.1   # Generate models for v1.1"
        ),
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Target protocol version to generate (e.g. 'v0.8', 'v0.9', 'v1.0', 'v1.1')"
        ),
    )
    args = parser.parse_args()

    dir_name, spec_dot, enum_name = normalize_version(args.version)
    print(
        f"Generating Pydantic models & adapters for {spec_dot} (module: {dir_name})..."
    )
    generate_version_schemas(dir_name, spec_dot)
    generate_basic_catalog(dir_name, spec_dot)
    generate_adapter_scaffold(dir_name, spec_dot, enum_name)
        ...
        for entry in sorted(os.listdir(schema_root)):
            if os.path.isdir(os.path.join(schema_root, entry)) and entry.startswith(
                ...
                if os.path.exists(os.path.join(schema_root, entry, "__init__.py")):
                    all_schema_dirs.append(normalize_version(entry))
