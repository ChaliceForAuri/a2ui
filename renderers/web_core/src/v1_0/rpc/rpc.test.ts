...
} from '../../v0_9/catalog/types.js';
import {DataContext} from '../../v0_9/rendering/data-context.js';
import {SurfaceModel} from '../../v0_9/state/surface-model.js';
...
describe('Stage 3 (Sauce-TS) Bidirectional RPC & @index Function Verification', () => {
  ...
  const functionsMap = new Map<string, FunctionImplementation>([
    ['customRpc', customRpcImpl],
    ['internalRenderer', rendererOnlyImpl],
    ['userActionOnly', restrictedImpl],
  ...
  const mockCatalog: Catalog<any> = {
    ...
    invoker: (funcName: string, args: any) => {
      const impl = functionsMap.get(funcName);
      if (!impl) throw new Error(`Function not found: ${funcName}`);
      return impl.execute(args, {} as any);
    },
  };
