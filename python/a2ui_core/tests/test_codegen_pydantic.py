...
def test_normalize_version():
    dir_name, spec_dot, enum_name = codegen_pydantic.normalize_version("v1.1")
    assert dir_name == "v1_1"
    assert spec_dot == "v1.1"
    assert enum_name == "V1_1"

    dir_name, spec_dot, enum_name = codegen_pydantic.normalize_version("1.2")
    assert dir_name == "v1_2"
    assert spec_dot == "v1.2"
    assert enum_name == "V1_2"

    dir_name, spec_dot, enum_name = codegen_pydantic.normalize_version("v0_9_1")
    assert dir_name == "v0_9_1"
    assert spec_dot == "v0.9.1"
    assert enum_name == "V0_9_1"

    dir_name, spec_dot, enum_name = codegen_pydantic.normalize_version("v2.0")
    assert dir_name == "v2_0"
    assert spec_dot == "v2.0"
    assert enum_name == "V2_0"


def test_map_json_type_to_python():
    codegen = codegen_pydantic.PydanticCodegen("v0_9", "v0.9")
...
def test_generate_basic_catalog_components():
    ...
    code, names = codegen_pydantic.generate_basic_catalog_components(mock_catalog_data)
    assert names == [
        "CatalogComponentCommon",
        "TextComponent",
        "AnyComponent",
...
def test_generate_basic_catalog_functions():
    ...
    code, names = codegen_pydantic.generate_basic_catalog_functions(mock_catalog_data)
    assert names == ["ToastArgs", "ToastApi", "BASIC_FUNCTIONS"]
...
def test_generate_basic_catalog_styles():
    ...
    code = codegen_pydantic.generate_basic_catalog_styles(mock_catalog_data)
...
def test_generate_server_to_client():
    ...
    code, names = codegen_pydantic.generate_server_to_client(mock_s2c_data)
...
def test_generate_schema_init():
    code = codegen_pydantic.generate_schema_init(["CreateSurfaceMessage"])
...
def test_generate_client_capabilities():
    ...
    code = codegen_pydantic.generate_client_capabilities(mock_capabilities_data)
...
def test_generate_client_to_server():
    ...
    code = codegen_pydantic.generate_client_to_server(mock_c2s_data)
    assert "class A2uiClientAction(StrictBaseModel):" in code
    assert "class A2uiValidationFailedError(StrictBaseModel):" in code
    assert (
    ...
    assert "A2uiClientError = Union[A2uiValidationFailedError]\n" in code
...
def test_compile_properties_required_with_default():
    codegen = codegen_pydantic.PydanticCodegen("v1_0", "v1.0")
    props = {
...
def test_generated_python_syntax_validity():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            versions = ["v0_8", "v0_9", "v1_0"]
            for ver in versions:
                dir_name, spec_dot, enum_name = codegen_pydantic.normalize_version(ver)
                codegen_pydantic.generate_version_schemas(dir_name, spec_dot)
                codegen_pydantic.generate_basic_catalog(dir_name, spec_dot)
                codegen_pydantic.generate_adapter_scaffold(
                    dir_name, spec_dot, enum_name
                )

            # Test root schema __init__.py update
            known_versions = [codegen_pydantic.normalize_version(v) for v in versions]
            codegen_pydantic.update_root_schema_init(known_versions, out_root=tmpdir)
